...
save( match_nomatch_vcf,
      file = file.path( root, "tmp/state_exact_match/panelmatch_nomatch_vcf.RData" ) )
toc()

# Before Refinement
# For before refinement, according to the original paper we should use mahalonobius
# but we are just taking the ps.weight
# do not get an specific match object for this panel of figure 5
tic()
match_none_vcf = PanelMatch(
  lag = 4, time.id = "year", unit.id = "cellid",
...
save(match_none_vcf,
     file = file.path(root, "tmp/state_exact_match/panelmatch_none_vcf.RData"))
toc()
######################################################
...
save(match_ps_vcf,
     file = file.path(root, "tmp/state_exact_match/panelmatch_ps_vcf.RData"))
toc()
######################################################
...
save(match_cbps_vcf,
     file = file.path(root, "tmp/state_exact_match/panelmatch_cbps_vcf.RData"))
...
save( match_nomatch_vcf, 
      ...
      match_cbps_vcf,
      df_match, 
      file = file.path(root, "tmp/state_exact_match/7_panelmatch_dist_state_exact.RData"))
