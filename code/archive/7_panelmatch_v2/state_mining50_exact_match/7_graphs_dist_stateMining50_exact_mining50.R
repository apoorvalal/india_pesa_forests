...
outsg_state_mining50_exact_match <- file.path(root, "out_sg/7_panelmatch_v2/state_mining50_exact_match")
...
load( file.path( state_mining_exact_match, 
...
png(file = file.path( outsg_state_mining50_exact_match , 
                      "distribution_con_Mining50.png") , 
    width = 4.25, height = 3.25, units = "in", res = 800 )

match_sizes_ps <- summary( match_ps_vcf_con_Mining50$att )$overview$matched.set.size
par( cex = 0.7 )
hist(match_sizes_ps, 
...
# Generating plot
png(file = file.path( outsg_state_mining50_exact_match, "fig5_con_Mining50.png"), 
...
# Generate Plot
png(file = file.path( outsg_state_mining50_exact_match, "fig4_con_Mining50.png") , 
...
(f2 = ggplot(df_aux1 %>% filter( variable == "Mining" ) ,
    ...
    scale_color_manual( values = c("red", "blue"),
                        labels = c( "Propensity Score", "Covariate Balancing Propensity Score" ) ) +
    labs(y = "Treatment Effect on Mining Conflict",
         x = "Years since PESA",
         colour = "") +
...
ggsave(file.path( outsg_state_mining50_exact_match, "fig7_con_Mining50.png"), 
       f2, device = png, width = 10, height = 6, dpi = 150, units = "in")
