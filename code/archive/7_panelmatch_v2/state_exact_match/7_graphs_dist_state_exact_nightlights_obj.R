...
state_exact_match <- file.path( tmp, "panel_match_v2/state_exact_match" )
...
load( file.path( state_exact_match, "panelmatch_cbps_vcf_nl_p95.RData" ) )
load( file.path( state_exact_match, "panelmatch_ps_vcf_nl_p95.RData" ) )
load( file.path( state_exact_match, "panelmatch_none_vcf_nl_p95.RData" ) )
...
land <- read.csv( file.path(land_conflict, "cleaned_data_v2.csv"))
toc()
...
balance_nomatch <- get_covariate_balance(match_none_vcf_nl_p95$att,
                                         data = df_match,
                                         covariates = cov_vars,
                                         plot = F,
                                         use.equal.weights = TRUE)
balance_norefinement <- get_covariate_balance(matched.sets = match_ps_vcf_nl_p95$att,
                                              data = df_match,
                                              covariates = cov_vars,
                                              use.equal.weights = TRUE)
balance_ps <- get_covariate_balance(match_ps_vcf_nl_p95$att,
                                    data = df_match,
                                    covariates = cov_vars,
                                    plot = F)
balance_cbps <- get_covariate_balance(match_cbps_vcf_nl_p95$att,
                                      data = df_match,
                                      covariates = cov_vars,
                                      plot = F )

# Save objects for plot
save_obj_path <- file.path( state_exact_match, "fig5_state_exact_nl_p95_obj.RData" )
...
# Generating Objects
matched_set_list <- list( match_ps_vcf_nl_p95$att,
                          match_cbps_vcf_nl_p95$att )
...
# Save objects for plot
save_obj_path <- file.path( state_exact_match, "fig4_state_exact_nl_p95_obj.RData" )
...
ps_results_vcf = PanelEstimate(sets = match_ps_vcf_nl_p95,
                               data = df_match,
                               number.iterations = 1000)
cbps_results_vcf = PanelEstimate(sets = match_cbps_vcf_nl_p95,
                                 data = df_match,
                                 number.iterations = 1000 )
# Get Variables
matchrestable = \(x, v) data.frame(v, with(x, cbind(lead, estimates, standard.error)))
variables_vec <- c("0nl_p95" )
...
df_aux1$variable <- factor(df_aux1$variable, levels = c("0nl_p95" ),
                           labels = c( "Nightlights" ) )


# Save objects for plot
save_obj_path <- file.path( state_exact_match, "fig7_state_exact_nl_p95_obj.RData" )
save( ps_results_vcf, 
      cbps_results_vcf, 
      df_aux1, 
      file = save_obj_path )
