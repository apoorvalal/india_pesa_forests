...
load( file.path( state_mining50_exact_match, "panelmatch_cbps_vcf_forest_index.RData" ) )
load( file.path( state_mining50_exact_match, "panelmatch_ps_vcf_forest_index.RData" ) )
load( file.path( state_mining50_exact_match, "panelmatch_none_vcf_forest_index.RData" ) )
...
df_final <- df_final %>% 
  ...
  select( cellid,  lag, forest_index, nl_p95, roadcomp_1, roadsanc_1 ) %>% 
  filter(lag < 0 & lag > -5 ) %>%
...
save(compared, 
     ...
     file = file.path( state_mining50_exact_match, 
                       "fig4_stateMining_exact_forest_index_obj.RData" ) )
